...
-- API接口元数据信息
CREATE TABLE IF NOT EXISTS yt_data_api_meta (
...
CREATE TABLE IF NOT EXISTS yt_data_api_request_cfg (
    ...
    `api_param_name` VARCHAR(32) NOT NULL COMMENT '请求参数名',
    `api_param_value` VARCHAR(32) NOT NULL COMMENT '请求参数值',
    `api_param_required` TINYINT NOT NULL COMMENT '请求参数是否必填, 0: 必填, 1: 非必填',
    `api_param_desc` VARCHAR(128) NOT NULL COMMENT '请求参数描述',
    `create_time` DATETIME NOT NULL COMMENT '创建时间',
    `update_time` DATETIME NOT NULL COMMENT '更新时间',
    INDEX api_id_index (`api_id`),
    UNIQUE INDEX api_param_name_index (`api_param_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS yt_data_api_security_cfg (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
    `api_id` INT(11) NOT NULL COMMENT '数据API接口ID',
    `api_limit_period` VARCHAR(32) NOT NULL COMMENT '数据接口访问限流周期, eg: 60sec',
    `api_limit_frequency` INT(11) NOT NULL COMMENT '数据接口访问限流频次',
    `create_time` DATETIME NOT NULL COMMENT '创建时间',
    `update_time` DATETIME NOT NULL COMMENT '更新时间',
    UNIQUE INDEX api_id_index (`api_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- 数据应用元数据
CREATE TABLE IF NOT EXISTS yt_data_application_meta (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
    ...
    `desc` VARCHAR(256) COMMENT '数据应用描述',
    `create_time` DATETIME NOT NULL COMMENT '创建时间',
    `update_time` DATETIME NOT NULL COMMENT '更新时间',
...
-- 数据应用已分配的数据API
CREATE TABLE IF NOT EXISTS yt_data_application_allocated_api (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
