...
-- 职位
CREATE TABLE IF NOT EXISTS yt_org_post_group (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
    `post_group_name` VARCHAR(64) NOT NULL COMMENT '簇名称',
    `post_group_code` VARCHAR(32) NOT NULL COMMENT '簇编码',
    `post_group_desc` VARCHAR(128) COMMENT '簇描述',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE INDEX job_group_name_index (`post_group_name`),
    UNIQUE INDEX job_group_code_index (`post_group_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS yt_org_post (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
    `post_group_id` INT(11) NOT NULL COMMENT '簇组',
    `post_name` VARCHAR(64) NOT NULL COMMENT '职位名成',
    `post_code` VARCHAR(32) NOT NULL COMMENT '职位编码',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE INDEX job_name_index (`post_name`),
    UNIQUE INDEX job_code_index (`post_code`),
    INDEX job_group_id_index (`post_group_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS yt_sys_resource (
    ...
    `permission_type` TINYINT NOT NULL COMMENT '权限类型, 0: 菜单, 1: 菜单页面, 2: 菜单页面动作资源',
...
CREATE TABLE IF NOT EXISTS yt_user_post (
    `id` INT(11) AUTO_INCREMENT PRIMARY KEY,
    `user_id` INT(11) NOT NULL COMMENT '用户ID',
